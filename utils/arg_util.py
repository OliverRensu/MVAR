...
class Args(Tap):
    ...
    data_path: str = '/datasets01/imagenet_full_size/061417/'
    exp_name: str = 'text'
...
def init_dist_and_get_args():
    for i in range(len(sys.argv)):
        if sys.argv[i].startswith('--local-rank=') or sys.argv[i].startswith('--local_rank='):
            del sys.argv[i]
            break
    args = Args(explicit_bool=True)
    args._add_argument("--ngpus", default=8, type=int, help="Number of gpus to request on each node")
    args._add_argument("--nodes", default=2, type=int, help="Number of nodes to request")
    args._add_argument("--timeout", default=4320, type=int, help="Duration of the job")
    args._add_argument("--job_dir", default="", type=str, help="Job dir. Leave empty for automatic.")

    args._add_argument("--partition", default="learnfair", type=str, help="Partition where to submit")
    args._add_argument('--comment', default="", type=str, help="Comment to pass to scheduler")
    args._add_argument('--world_size', default=1, type=int,
                        help='number of distributed processes')
    args._add_argument('--local-rank', default=-1, type=int)
    #args._add_argument('--dist_on_itp', action='store_true')
    args._add_argument('--dist_url', default='env://',
