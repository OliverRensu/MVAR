...
import dist
from models import MVAR, VQVAE, VectorQuantizer2
...
class MVARTrainer(object):
    def __init__(
        ...
        vae_local: VQVAE, mvar_wo_ddp: MVAR, mvar: DDP,
        mvar_opt: AmpOptimizer, label_smooth: float,
    ):
        super(MVARTrainer, self).__init__()
        
        self.mvar, self.vae_local, self.quantize_local = mvar, vae_local, vae_local.quantize
    ...
    def eval_ep(self, ld_val: DataLoader):
        ...
        for inp_B3HW, label_B in ld_val:
            ...
            logits_BLV = self.mvar_wo_ddp(label_B, x_BLCv_wo_first_l)
    ...
    def train_step(
        ...
        self.mvar_wo_ddp.prog_si = self.vae_local.quantize.prog_si = prog_si
        ...
        with self.mvar_opt.amp_ctx:
            ...
            logits_BLV = self.mvar(label_B, x_BLCv_wo_first_l)
        ...
        # backward
        _, scale_log2 = self.mvar_opt.backward_clip_step(loss=loss, stepping=stepping)
        ...
        if it == 0 or it in metric_lg.log_iters:
            ...
            metric_lg.update(Lm=Lmean, Lt=Ltail, Accm=acc_mean, Acct=acc_tail, tnm=0)
        ...
        self.mvar_wo_ddp.prog_si = self.vae_local.quantize.prog_si = -1
        return 0., scale_log2
    ...
    def state_dict(self):
        ...
        for k in ('mvar_wo_ddp', 'vae_local', 'mvar_opt'):
            m = getattr(self, k)
            if m is not None:
