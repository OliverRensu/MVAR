...
class MVAR(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            AdaLNSelfAttn(
                ...
                flash_if_available=flash_if_available, fused_if_available=fused_if_available,patch_nums=patch_nums
            )
        ...
        print(
            ...
            f'    [MVAR config ] embed_dim={embed_dim}, num_heads={num_heads}, depth={depth}, mlp_ratio={mlp_ratio}\n'
        ...
        start = 0
        attention_mask = []
        total_length = sum([pz * pz for pz in self.patch_nums])
        for pz in self.patch_nums:
            start += pz ** 2
            attention_mask.append(torch.cat([torch.zeros((pz * pz, start - pz ** 2)), torch.ones((pz * pz, pz ** 2)),
                                             torch.zeros((pz * pz, total_length - start))], dim=-1))
        ...
        self.register_buffer('attn_bias_for_masking', attention_mask.contiguous())
    ...
    def autoregressive_infer_cfg(
        ...
        next_token_map = sos.unsqueeze(1).expand(2 * B, self.first_l, -1) + self.pos_start.expand(2 * B,
                                                                                                  self.first_l,
                                                                                                  -1) + lvl_pos[:,
                                                                                                        :self.first_l]
        input_sequence = next_token_map
        cur_L = 0
        f_hat = sos.new_zeros(B, self.Cvae, self.patch_nums[-1], self.patch_nums[-1])

        for b in self.blocks: b.cache = None
        for si, pn in enumerate(self.patch_nums):  # si: i-th segment
            ...
            for b in self.blocks:
                x = b(x=x, cond_BD=cond_BD_or_gss, attn_bias=self.attn_bias_for_masking[:, :, :cur_L, :cur_L])
